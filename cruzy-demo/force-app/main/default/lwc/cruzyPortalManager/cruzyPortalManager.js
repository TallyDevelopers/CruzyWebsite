...
import getPortalStatus from '@salesforce/apex/CruzyPortalManagerController.getPortalStatus';
import setPortalStatus from '@salesforce/apex/CruzyPortalManagerController.setPortalStatus';
import generateResetLink from '@salesforce/apex/CruzyPortalManagerController.generateResetLink';
import enrollMember from '@salesforce/apex/CruzyPortalManagerController.enrollMember';
import setTemporaryPassword from '@salesforce/apex/CruzyPortalManagerController.setTemporaryPassword';

export default class CruzyPortalManager extends LightningElement {
    @api recordId;
    ...
    @wire(getPortalStatus, { contactId: '$recordId' })
    wiredPortalStatus(result) {
        ...
        if (result.data) {
            ...
            this.errorMsg = 'Failed to load portal status. ' + (result.error.body?.message || '');
            this.isLoading = false;
        }
    }
    ...
    get memberIdDisplay() {
        return this.portalData?.memberId || 'Not enrolled';
    }
    ...
    get resetExpiryDisplay() {
        ...
        return exp.toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' });
    }
    ...
    get isActive() {
        return this.portalData?.portalStatus === 'Active';
    }

    get statusToggleTitle() {
        return this.isActive ? 'Deactivate Portal Access' : 'Activate Portal Access';
    }

    get statusToggleDesc() {
        return this.isActive
            ? 'Immediately blocks this member from logging in'
    ...
    get statusToggleVariant() {
        return this.isActive ? 'destructive' : 'success';
    }

    get statusToggleIcon() {
        return this.isActive ? 'utility:ban' : 'utility:check';
    }

    get tempPasswordInputType() {
        return this.showTempPassword ? 'text' : 'password';
    }

    get tempPasswordToggleIcon() {
        return this.showTempPassword ? 'utility:hide' : 'utility:preview';
    }

    get tempPasswordToggleLabel() {
        return this.showTempPassword ? 'Hide' : 'Show';
    }

    get isSetPasswordDisabled() {
        return this.isBusy || !this.tempPassword || this.tempPassword.length < 8;
    }
    ...
    async handleEnroll() {
        ...
        this.isBusy = true;
        try {
            const result = await enrollMember({ contactId: this.recordId });
            if (result.startsWith('already_enrolled:')) {
                this.errorMsg = `This contact is already a Cruzy+ member (${result.split(':')[1]}).`;
            } else {
                const parts = result.split(':');
                const memberId = parts[1];
                const link = parts.slice(2).join(':');
                this.successMsg = `✓ Enrolled! Member ID: ${memberId}. Welcome email sent.`;
                if (link) this.generatedLink = link;
                this.dispatchEvent(new ShowToastEvent({
                    title: 'Member Enrolled',
                    message: `Cruzy+ membership created. Member ID: ${memberId}`,
                    variant: 'success'
                }));
            }
    ...
    async handleStatusToggle() {
        ...
        try {
            ...
            this.dispatchEvent(new ShowToastEvent({
                ...
                message: `Portal access is now ${newStatus}.`,
    ...
    handleTempPasswordChange(event) {
        ...
        if (this.tempPassword.length > 0 && this.tempPassword.length < 8) {
    ...
    async handleSetTempPassword() {
        ...
        try {
            ...
            this.successMsg = `✓ Temporary password set and emailed to ${this.portalData.email}. Member status set to Active.`;
            this.dispatchEvent(new ShowToastEvent({
                title: 'Password Set',
                message: 'Temporary password set and notification email sent.',
                variant: 'success'
            }));
            await refreshApex(this._wiredResult);
        } catch (e) {
