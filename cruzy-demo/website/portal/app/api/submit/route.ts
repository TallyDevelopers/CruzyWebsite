...
export async function POST(req: NextRequest) {
  try {
    ...
    // 5. Send welcome email with portal set-password link
    if (sfResult?.contactId) {
      try {
        const token = generateResetToken()
        const expiry = new Date()
        expiry.setHours(expiry.getHours() + 24)

        await updateContact(sfResult.contactId, {
          Portal_Status__c: 'Pending',
          Portal_Reset_Token__c: token,
          Portal_Reset_Expiry__c: expiry.toISOString(),
        })

        const setPasswordUrl = `${process.env.NEXT_PUBLIC_PORTAL_URL || 'http://localhost:3000'}/set-password?token=${token}`
        const { subject, html } = welcomeEmail(firstName, setPasswordUrl, cruzyMemberId)
        await sendEmail(email, subject, html)
      } catch (emailErr) {
        console.error('Welcome email failed (signup still succeeded):', emailErr)
      }
    }
