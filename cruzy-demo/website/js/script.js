...
// Cruzy Auth State: check portal login and update header accordingly
(function() {
  var PORTAL = (typeof CRUZY !== 'undefined' && CRUZY.portalUrl) ? CRUZY.portalUrl.replace(/\/+$/, '') : 'http://localhost:3000';
  fetch(PORTAL + '/api/auth/me', { credentials: 'include' })
    .then(function(r) { return r.json(); })
    .then(function(data) {
      if (!data.loggedIn) return;

      // Update desktop "Login" link
      var loginLinks = document.querySelectorAll('a.my_account');
      loginLinks.forEach(function(el) {
        el.textContent = 'My Account';
        el.href = PORTAL + '/account';
      });

      // Update desktop "Join Cruzy+" button (second .ext_btn a)
      var extBtns = document.querySelectorAll('.ext_btn.not_mob a.hvr-shutter-in-vertical');
      if (extBtns.length >= 2) {
        var joinBtn = extBtns[1];
        joinBtn.href = PORTAL + '/account';
        joinBtn.innerHTML = '<span class="button-text">My</span><span class="button-text">Account</span>';
      }

      // Update mobile "Login" in mob_lnks
      var mobLogin = document.querySelector('.mob_lnks .my_account');
      if (mobLogin) {
        mobLogin.textContent = 'My Account';
        mobLogin.href = PORTAL + '/account';
      }

      // Update mobile "Join Cruzy+" button
      var mobBtns = document.querySelectorAll('.mob_lnks .ext_btn a');
      mobBtns.forEach(function(el) {
        if (el.textContent.trim() === 'Join Cruzy+') {
          el.textContent = 'My Account';
          el.href = PORTAL + '/account';
        }
      });

      // Update CTA "Join Cruzy+" buttons in page body
      var ctaJoins = document.querySelectorAll('a.cta-join-btn');
      ctaJoins.forEach(function(el) {
        el.innerHTML = '<span>My Account</span>';
        el.href = PORTAL + '/account';
      });
    })
    .catch(function() { /* not logged in or portal down */ });
})();